...
══════════════════════════════════════════════ */
let playerStats = { wins: 0, losses: 0, draws: 0, played: 0, streak: 0, lastLogin: null, questsDate: null, quests: [], achievements: {} };
const ACHIEVEMENTS_INFO = { first_blood: { name: 'Primera Sangre', icon: '🩸' }, iron_wall: { name: 'Muro Infranqueable', icon: '🛡' }, defender_win: { name: 'Rey a Salvo', icon: '👑' }, invader_win: { name: 'El Asedio', icon: '⚔' }, speedrun: { name: 'Guerra Relámpago', icon: '⚡' } };
const QUEST_POOL = [{ id: 'play_3', type: 'play', target: 3, desc: 'Juega 3 partidas' }, { id: 'capture_5', type: 'capture', target: 5, desc: 'Captura 5 piezas enemigas' }, { id: 'win_1', type: 'win', target: 1, desc: 'Consigue 1 victoria' }, { id: 'iron_2', type: 'iron', target: 2, desc: 'Forma 2 Líneas de Hierro' }];

function initStats() {
  try { const saved = localStorage.getItem('dominio_player_stats'); if (saved) { playerStats = { ...playerStats, ...JSON.parse(saved) }; if (!playerStats.achievements) playerStats.achievements = {}; if (!playerStats.quests) playerStats.quests = []; } } catch(e) {}
  checkDailyTasks(); updateStatsUI();
}
function saveStats() { try { localStorage.setItem('dominio_player_stats', JSON.stringify(playerStats)); } catch(e) {} updateStatsUI(); }
function updateStatsUI() { if (document.getElementById('stat-played')) document.getElementById('stat-played').textContent = playerStats.played; if (document.getElementById('stat-wins')) document.getElementById('stat-wins').textContent = playerStats.wins; if (document.getElementById('streak-display')) document.getElementById('streak-display').textContent = playerStats.streak; }
function checkDailyTasks() { const d = new Date(), today = `${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`; if (playerStats.lastLogin !== today) { if (playerStats.lastLogin) { const yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1); const yStr = `${yesterday.getFullYear()}-${yesterday.getMonth()}-${yesterday.getDate()}`; if (playerStats.lastLogin === yStr) playerStats.streak++; else playerStats.streak = 1; } else playerStats.streak = 1; playerStats.lastLogin = today; } if (playerStats.questsDate !== today || !playerStats.quests.length) { playerStats.questsDate = today; let shuffled = [...QUEST_POOL].sort(() => 0.5 - Math.random()); playerStats.quests = shuffled.slice(0, 3).map(q => ({ ...q, progress: 0, done: false })); } saveStats(); }
function trackQuest(type, amount = 1) { if (isAITurn() && gameMode !== 'eve') return; let updated = false; playerStats.quests.forEach(q => { if (q.type === type && !q.done) { q.progress += amount; if (q.progress >= q.target) { q.progress = q.target; q.done = true; showToast('Misión Completada', q.desc, '📜'); } updated = true; } }); if (updated) saveStats(); }
function unlockAchievement(id) { if (isAITurn() && gameMode !== 'eve') return; if (!playerStats.achievements[id]) { playerStats.achievements[id] = true; saveStats(); const ach = ACHIEVEMENTS_INFO[id]; showToast('¡Logro Desbloqueado!', ach.name, ach.icon); } }
function openQuests() { document.getElementById('start-overlay').classList.remove('show'); document.getElementById('quests-overlay').classList.add('show'); const qCont = document.getElementById('quests-container'); qCont.innerHTML = ''; playerStats.quests.forEach(q => { const pct = Math.min(100, Math.round((q.progress / q.target) * 100)); qCont.innerHTML += `<div class="quest-card ${q.done ? 'done' : ''}"><div class="q-title">${q.desc}</div><div class="q-bar-bg"><div class="q-bar-fill" style="width:${pct}%"></div></div><div style="font-size:0.7rem; color:var(--text-dim); text-align:right;">${q.progress} / ${q.target}</div></div>`; }); const aCont = document.getElementById('achiev-container'); aCont.innerHTML = ''; for (const [id, info] of Object.entries(ACHIEVEMENTS_INFO)) { const isUnlocked = playerStats.achievements[id]; aCont.innerHTML += `<div class="achiev-card ${isUnlocked ? 'unlocked' : ''}"><div class="achiev-icon">${info.icon}</div><div class="achiev-name">${info.name}</div><div class="achiev-desc">${info.desc}</div></div>`; } }
...
const TUT_STEPS = [{ icon: '🏰', title: 'Bienvenido a Dominio', text: 'Dominio es un juego de asedio. Un bando defiende el castillo y el otro ataca.' }, { icon: '⚔', title: 'Los Invasores', text: 'Los Invasores atacan. Se mueven 1 ó 2 casillas libres en línea recta.' }, { icon: '🛡', title: 'Los Guardianes', text: 'Los Guardianes protegen al Señor. Se mueven 1 casilla libre en línea recta.' }, { icon: '♛', title: 'El Señor del Hierro', text: 'El Señor empieza en el centro y es invulnerable ahí. Se mueve 1 casilla, o salta sobre un Guardián propio.' }, { icon: '⚖', title: 'Cómo Capturar', text: 'Una pieza es capturada al quedar flanqueada entre dos piezas enemigas en línea recta.' }, { icon: '🏆', title: 'Victoria', text: '• Defensor gana si el Señor llega a una esquina ✦.\n• Invasor gana si captura al Señor fuera de D4.' }];
...
function renderTutStep() { const step = TUT_STEPS[tutCurrentStep]; document.getElementById('tut-content').innerHTML = `<div class="tut-step active"><div class="tut-title">${step.title}</div><div class="tut-icon">${step.icon}</div><div class="tut-text" style="white-space: pre-line;">${step.text}</div></div>`; const dots = document.getElementById('tut-dots'); dots.innerHTML = ''; TUT_STEPS.forEach((_, i) => { const dot = document.createElement('div'); dot.className = 'tut-dot' + (i === tutCurrentStep ? ' active' : ''); dot.onclick = () => { tutCurrentStep = i; renderTutStep(); }; dots.appendChild(dot); }); const prevBtn = document.getElementById('btn-tut-prev'), nextBtn = document.getElementById('btn-tut-next'); prevBtn.disabled = tutCurrentStep === 0; prevBtn.style.opacity = tutCurrentStep === 0 ? '0.35' : '1'; if (tutCurrentStep === TUT_STEPS.length - 1) { nextBtn.textContent = '¡A Jugar! ⚔'; nextBtn.onclick = closeTutorial; } else { nextBtn.textContent = 'Siguiente'; nextBtn.onclick = nextTut; } }
...
function showToast(title, desc, icon) { const container = document.getElementById('toast-container'); const toast = document.createElement('div'); toast.className = 'toast'; toast.innerHTML = `<div class="toast-icon">${icon}</div><div class="toast-body"><div class="toast-title">${title}</div><div class="toast-desc">${desc}</div></div>`; container.appendChild(toast); setTimeout(() => { toast.classList.add('hide'); setTimeout(() => { if(toast.parentNode) toast.remove(); }, 400); }, 4000); }
...
function selectMenuBot(botId) {
  ...
  if (assistSelect) {
    if (botId === 'darius') { assistSelect.value = 'challenge'; assistSelect.disabled = true; } 
    else { assistSelect.disabled = false; }
    updateAssistInfo();
  }
}
...
function startGame(mode, botId = null) {
  ...
  boardHistory = [getBoardHash()]; stateHistory = []; historyIndex = 0; lastFrom = null; lastTo = null; animateNextMove = false;
  ironSet = calculateIronSetSim(board);
  
  if (mode === 'pvp') { undosLeft = Infinity; } 
  else {
...
function calculateIronSetSim(boardState) { let iset = new Set(); for (let r = 0; r < ROWS; r++) { let run = []; for (let c = 0; c <= COLS; c++) { if (c < COLS && boardState[r][c] === 'G') run.push(`${r},${c}`); else { if (run.length >= 3) run.forEach(k => iset.add(k)); run = []; } } } for (let c = 0; c < COLS; c++) { let run = []; for (let r = 0; r <= ROWS; r++) { if (r < ROWS && boardState[r][c] === 'G') run.push(`${r},${c}`); else { if (run.length >= 3) run.forEach(k => iset.add(k)); run = []; } } } return iset; }
...
function getValidMovesSim(boardState, r, c, currentIronSet, isKingOut) { const p = boardState[r][c]; if (p === 'S') return getKingMovesSim(boardState, r, c, isKingOut); if (p === 'G') return getGuardianMovesSim(boardState, r, c); if (p === 'I') return getInvaderMovesSim(boardState, r, c, currentIronSet); return []; }
function getKingMovesSim(boardState, r, c, isKingOut) { const moves = []; for (const [dr, dc] of DIRS) { const r1 = r + dr, c1 = c + dc; if (!inBounds(r1, c1)) continue; if (boardState[r1][c1] === '.') { if (isKingOut && r1 === CR && c1 === CC) continue; moves.push({ r: r1, c: c1 }); } else if (boardState[r1][c1] === 'G') { const r2 = r + dr * 2, c2 = c + dc * 2; if (inBounds(r2, c2) && boardState[r2][c2] === '.') { if (isKingOut && r2 === CR && c2 === CC) continue; moves.push({ r: r2, c: c2 }); } } } return moves; }
function getGuardianMovesSim(boardState, r, c) { const moves = []; for (const [dr, dc] of DIRS) { const nr = r + dr, nc = c + dc; if (inBounds(nr, nc) && boardState[nr][nc] === '.' && !isCorner(nr, nc)) moves.push({ r: nr, c: nc }); } return moves; }
function getInvaderMovesSim(boardState, r, c, currentIronSet) { const maxDistance = isAdjToIronSim(r, c, currentIronSet) ? 1 : 2; const moves = []; for (const [dr, dc] of DIRS) { for (let d = 1; d <= maxDistance; d++) { const nr = r + dr * d, nc = c + dc * d; if (!inBounds(nr, nc) || isCorner(nr, nc) || boardState[nr][nc] !== '.') break; moves.push({ r: nr, c: nc }); } } return moves; }
...
function undoMove() {
  ...
  if (undosLeft <= 0) { showToast("Sin Deshacer", "El destino de estas piezas ya está sellado.", "🚫"); return; }

  let stepsToPop = 1;
  if (activeBotId && !isAITurn() && stateHistory.length >= 3) { stepsToPop = 2; }
  if (undosLeft !== Infinity) undosLeft--;

  for(let i = 0; i < stepsToPop; i++){ stateHistory.pop(); boardHistory.pop(); }
  historyIndex = stateHistory.length - 1;
  const s = stateHistory[historyIndex];
  
  board = s.board.map(r => [...r]); currentPlayer = s.currentPlayer; kingLeftCenter = s.kingLeftCenter; round = s.round; phase = s.phase; ironSet = new Set(s.ironSet); isGameOver = s.isGameOver; lastFrom = s.lastFrom; lastTo = s.lastTo;
...
function updateUI() {
  ...
  const badge = document.getElementById('turn-badge'); const ovl = document.getElementById('past-overlay');
...
function loadState(idx) {
  if (idx < 0 || idx >= stateHistory.length) return;
  historyIndex = idx; const s = stateHistory[idx];
  board = s.board.map(r => [...r]); currentPlayer = s.currentPlayer; kingLeftCenter = s.kingLeftCenter; round = s.round; phase = s.phase; maxRounds = s.maxRounds || 20; ironSet = new Set(s.ironSet); isGameOver = s.isGameOver; lastFrom = s.lastFrom; lastTo = s.lastTo;
  animateNextMove = true; updateUI(); renderLog();
  if (historyIndex === stateHistory.length - 1 && !isGameOver && isAITurn() && !isProcessingAI) checkAndTriggerAI();
}
...
function getBotDialogue(event) { if (!activeBotId) return null; const bot = BOTS[activeBotId]; if (!bot) return null; if (dariusDialogueCooldown > 0) { dariusDialogueCooldown--; return null; } const lines = bot.dialogue[event]; if (!lines || !lines.length) return null; dariusDialogueCooldown = 2 + Math.floor(Math.random() * 2); return lines[Math.floor(Math.random() * lines.length)]; }
function showBotDialogue(event) { const line = getBotDialogue(event); if (!line || !activeBotId) return; const chatBubble = document.getElementById('in-game-bot-chat'); if (chatBubble) { chatBubble.textContent = line; chatBubble.classList.remove('pop-anim'); void chatBubble.offsetWidth; chatBubble.classList.add('pop-anim'); } }
